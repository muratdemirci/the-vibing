...
function Post({ id, post, postPage }) {
  ...
  //TODO: add view count here   setViews(views + 1)
  //TODO: add share count here   setShares(shares + 1)
  useEffect(
  ...
  return (
    <div
      ...
      //TODO: redirect to status page https://twitter.com/WSJ/status/1647393139974348804
      // domain.com/usertag/status/id
      onClick={() => router.push(`/${id}`)}
    >
      {!postPage && <img src={post?.userImg} alt="" className="h-11 w-11 rounded-full mr-4" />}
      <div className={`${lora.className} flex flex-col space-y-2 w-full`}>
        <div className={`flex ${!postPage && 'justify-between'}`}>
          ...
          <div className="icon group flex-shrink-0 ml-auto">
            <DotsHorizontalIcon className="h-5 text-[#6e767d] group-hover:text-[#8782de]" />
          </div>
        </div>
        {postPage && <p className="text-black mt-0.5 text-xl">{post?.text}</p>}
        ...
        <div className={`text-[#6e767d] flex justify-between w-10/12 ${postPage && 'mx-auto'}`}>
          ...
          >
            <div className="icon group-hover:bg-[#8782de] group-hover:bg-opacity-10">
              <ChatIcon className="h-5 group-hover:text-[#8782de]" />
            </div>
            {comments.length > 0 && (
              <span className="group-hover:text-[#8782de] text-sm">{comments.length}</span>
            )}
          </div>
          ...
          <div className="icon group">
            <ShareIcon className="h-5 group-hover:text-[#8782de]" />
          </div>
          <div className="icon group">
            <ChartBarIcon className="h-5 group-hover:text-[#8782de]" />
          </div>
        </div>
      </div>
    </div>
